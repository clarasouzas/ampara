<img src="" width="300" height="400">

## Sobre

AMPARA é uma plataforma web desenvolvida para promover acolhimento, informação e apoio às participantes do projeto de pesquisa **Tecendo Redes de Cuidado**, realizado em parceria com o Núcleo de Gênero e Diversidade (NUGEDI) do IFRN – Campus São Paulo do Potengi. O sistema busca fortalecer redes de apoio e oferecer um ambiente digital seguro para interação, escuta e compartilhamento de informações relacionadas ao enfrentamento da violência de gênero.


## Tecnologias Utilizadas
[![SkillIcons](https://skillicons.dev/icons?i=js,py,html,css,django,bootstrap)](https://skillicons.dev)<br/>

## Integrantes
- Anny Carolline
- Maria Clara
- Williany Vitória

## Rodando
Siga as instruções abaixo para configurar o ambiente do projeto após clonar o repositório.

### 1. Criando e Ativando o Ambiente Virtual
```bash
python -m venv venv  
venv\Scripts\Activate.ps1
```

### 2. Instalando as Dependências
```bash
pip install -r requirements.txt  
```

### 3. Aplicando as Migrações
```bash
python manage.py migrate  
```

### 4. Criando um Superusuário
```bash
python manage.py createsuperuser  
```

Siga as instruções e defina um nome de usuário, e-mail e senha.

### 5. Executando o Servidor

```bash
python manage.py runserver  
```

### 6. Acessando o Ampara

Abra o navegador e acesse:

```
http://127.0.0.1:8000/
```

Faça login com o superusuário criado anteriormente.

Agora o **Gabarita** está pronto para ser utilizado!
